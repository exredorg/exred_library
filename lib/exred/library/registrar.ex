defmodule Exred.Library.Registrar do
  ...
  def register(module_name, node_attributes) do
    ...
    node = %Node{
      ...
      info: node_attributes.info,
      config: node_attributes.config,
      ui_attributes: node_attributes.ui_attributes,
      is_prototype: true
    }

    case SqliteRepo.insert(node, on_conflict: :nothing) do
      {:ok, _} -> :ok
      {:error, _} -> :already_registered
    end
